...
async def validate_installation():
    ...
    for dir in config.config.test_dir.iterdir():
        ...
        if str(dir).startswith('.'):
            continue
        if not os.path.isfile(dir / config.config.scheme_name) or not os.path.isfile(dir / config.config.test_config_name):
            continue
        print(f"Got to test {dir}")

async def validate_simulation(imn_file, config_filepath, parallel):
...
# ---- LOGIC FROM PREVIOUS MAIN FUNCTION ----
async def ex_main(imn_file, config_filepath, verbose, parallel):
    config.config.imunes_filename = imn_file
    config.config.test_config_filename = config_filepath
    config.config.set_platform()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--validate-installation", action="store_true",
                        help="Validate imunes installation itself (use instead of file args)")

    args = parser.parse_args()

    # Validate mutually exclusive modes:
    if args.validate_installation and (args.imn_file is not None or args.config_file is not None):
        parser.error("When using --aggregate-run, do not provide imn_file or config_file.")
